...
property_double (vignette_size, _("Vignette Size"), 20.00)
    value_range (0.00, 100.00)
            
#else

#define GEGL_OP_COMPOSER
#define GEGL_OP_NAME     gmic_fx_lomo
#define GEGL_OP_C_SOURCE fx_lomo.c
         ...
         gint level)
{
    ...
    char full_cmd[2048];
    snprintf(full_cmd, sizeof(full_cmd), "%s %s gui_merge_layers", "fx_lomo", properties_string(props));
...
gegl_op_class_init (GeglOpClass *klass)
{
  ...
  gegl_operation_class_set_keys (operation_class,
    ...
    "title",       _("Lomo"),
    "categories",  "generic",
    "reference-hash", "gmic_fx_lomo",
    "gimp:menu-path", "<Image>/Filters/G'MIC GEGL/",
    "gimp:menu-label", _("Lomo"),
    NULL);
}
